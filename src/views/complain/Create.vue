<template>
  <div class="complain-create">
    ...
    <div class="complain-plan-create">
      ...
      <div class="d-flex mt-3">
        ...
        <div class="button-upload-1988 d-flex align-items-center">
          <div class="choose-img" v-if="loadingImageBill">
            <i class="el-icon-loading"></i>
          </div>
    ...
    <div class="complain-create-table-wrap-v2-1976 mx-3 my-4">
      ...
      <!-- Ảnh hàng nhận dc  -->
      <div class="line-wrap d-flex align-items-center">
        <div class="choose-img" v-if="loadingImage">
          <i class="el-icon-loading"></i>
        </div>
        <div class="choose-img" v-else @click="selectImageReceived()">
          <img src="@/static/icon/complain-create/add.svg" alt="" />
        </div>
        <div
          class="image-wrap-Rec d-flex flex-column"
          v-for="i in this.modelData.img_tempReceived"
          :key="i.full_path"
        >
          <!-- <img :src="i.full_path" /> -->
          <img v-if="checkImage(i)" :src="i.full_path" />
...
export default {
  ...
  data() {
    return {
      ...
      count: 1,
      loadingImage: false,
      loadingImageBill: false,
      loadingImageRec: false,
      //
      images_temps: [],
      //
      modelData: {
  ...
  methods: {
    ...
    handleFileUpload() {
      ...
      if (!arr_type.includes(typeFile)) {
        this.$message.error("Chỉ được upload file ảnh & video");
        return;
      }
      this.submitFile();
    },
    ...
    clearImageRec(item) {
      var index = this.modelData.img_tempReceived.indexOf(item);
      if (index !== -1) {
        this.modelData.img_tempReceived.splice(index, 1);
        this.$message("Xóa thành công");
      }
    },
    selectImageReceived() {
    ...
    async submitFileReceived() {
      ...
      fetch(process.env.VUE_APP_API_ADMIN_URL + "media/upload", {
        ...
        .then((response) => response.json())
        .then((result) => {
          this.loadingImageRec = false;
          this.modelData.img_tempReceived.push(result.data[0]);
        })
        .catch(() => {
          this.packClick.loading = false;
          this.$notify.error({
        ...
        .then((response) => {
          switch (response.code) {
            case 200:
              ...
              console.log("this", this.ListComplain);
    ...
    submitForm() {
      ...
      let images_Rec = [];
      this.canSubmit = false;
      this.modelData.img_tempReceived.forEach((item) => {
        images_Rec.push(item.path);
      });
      this.modelData.img_Received = images_Rec;
      let data = {
        order_package_id: this.$route.params.pack_id,
        complain_type_id: this.ListType.id,
        solution_id: this.ListSolution.id,
        products: this.order_detail,
        images_received: this.modelData.img_Received,
...
.complain-create .is-disabled input {
  color: #000 !important;
}
</style>
