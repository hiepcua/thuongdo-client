  ...
  <div class="complain-create">
      ...
      <div class="d-flex mt-3">
        ...
        <div class="button-upload-1988">
          <el-upload
            ...
            :file-list="fileList_bill"
            :on-change="handleChange"
            list-type="text"
          >
            <button><span>Tải lên</span></button>
          </el-upload>
    ...
    <div class="complain-create-table-wrap-v2-1976 mx-3 my-4">
      ...
      <div class="line-wrap">
        <div class="d-flex align-items-center">
          ...
          <div class="choose-img" v-else @click="selectImage2()">
            <img src="@/static/icon/complain-create/add.svg" alt="" />
          </div>
          <div
            class="uploadFile_2 d-flex flex-column align-items-center"
            v-for="item in this.imagesData.images_received"
            :key="item.full_path"
          >
            <div class="ml-1">
              <img v-if="checkImage(item)" :src="item.full_path" />
              <div v-else>
                <video controls="controls">
                  <source :src="item.full_path" />
                </video>
              </div>
            </div>
            <img
    ...
    <div>
      <input
        ...
        id="fileUpload"
        ref="file2"
        v-on:change="handleFileUploadV2()"
      />
    </div>
    <!-- Input upload ảnh -->
    <div>
...
export default {
  ...
  methods: {
    ...
    clearImage2(item) {
      var index = this.imagesData.images_received.indexOf(item);
      if (index !== -1) {
        this.imagesData.images_received.splice(index, 1);
        this.$message("Xóa thành công");
      }
    },
    // Xử lý upload ảnh
    ...
    async submitFile() {
      ...
      fetch(process.env.VUE_APP_API_ADMIN_URL + "media/upload", {
        ...
        .then((result) => {
          ...
          console.log("this", this.packClick.images.images_temp);
        })
        .catch(() => {
    ...
    async submitFile2() {
      ...
      fetch(process.env.VUE_APP_API_ADMIN_URL + "media/upload", {
        ...
        .then((result) => {
          this.loadingImage = false;
          this.imagesData.images_received.push(result.data[0]);
          console.log("list", this.imagesData.images_received);
        })
        .catch(() => {
          this.packClick.loading = false;
          this.$notify.error({
    ...
    //
    handlePreview(file) {
      this.dialogImageUrl = file.response.data.full_path;
    ...
    handleChange(file, fileList_bill) {
      this.dataFile = fileList_bill;
      this.FileImage_bill = [];
      this.FileImage_billV1 = [];
      this.images_bill = [];
      this.images_bill_id = [];
      this.images_bill_all = [];
      this.dataFile.forEach((item) => {
        if (item.response != undefined) {
          this.FileImage_bill.push(item.response);
          this.FileImage_bill.forEach((item2) => {
            this.FileImage_billV1 = item2.data;
          });
          this.images_bill.push(this.FileImage_billV1);
          this.images_bill.forEach((item3) => {
            this.images_bill_id = item3.path;
          });
          this.images_bill_all.push(this.images_bill_id);
        }
      });
    },
    // Ảnh vận đơn
    ...
    submitForm() {
      ...
      //
      let images_temp3 = [];
      this.imagesData.images_received.forEach((item) => {
        images_temp3.push(item.path);
      });
      this.imagesData.images_received = images_temp3;

      let data = {
        complain_type_id: this.ListType.id,
        solution_id: this.ListSolution.id,
        products: this.order_detail,
        images_received: this.imagesData.images_received,
        images_bill: this.images_bill_all,
      };
      axios
...
.button-upload-1988 button:hover {
  background: #fff;
  color: #26b01c;
}
