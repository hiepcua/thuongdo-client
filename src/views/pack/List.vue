<template>
  <div class="frame-content-common-2022 pack-list" id="ST2022">
    ...
    <div class="pack-list-filter">
      <div class="d-flex">
        <div class="frame-filter-item-2022 pack-list-status-10">
          <span class="text" @click="getPackToVN()">Tổng số kiện đã về VN</span>
          <span class="count">{{ countPackToVN }}</span>
          <img
            src="@/static/icon/pack/list/bx_x-circle.svg"
            v-if="showPackToVN"
      ...
      <div @click="viewCreateDialog">
        <span class="frame-btn-add-filter-x-2022">Tạo giao hàng</span>
      </div>
    </div>
    <div class="table-element-custom-2022 pack-list-table" v-loading="loading">
      ...
      >
        ...
        <el-table-column label="Loại hàng" width="120">
          <template slot-scope="scope">
            <div class="d-flex flex-column">
              <div>
                <span
        ...
        <el-table-column label="Phí vận chuyển nội địa TQ (vnd)" width="150">
          <template slot-scope="scope">
            <div class="d-flex flex-column">
        ...
        <el-table-column label="Phí vận chuyển quốc tế (vnd)" width="150">
          <template slot-scope="scope">
            <div class="d-flex flex-column">
    ...
    <div
      ...
      v-if="listPack.length && showPackToVN == false"
    >
      <el-pagination
        background
    ...
    <div class="pack-to-VN" v-if="showPackToVN">
      <div class="table-element-custom-2022 pack-to-VN-table-89">
        <el-table :data="reportsPackToVN.orders" style="width: 100%">
          <el-table-column prop="time" label="Ngảy đặt hàng"></el-table-column>
          <el-table-column prop="code" label="Mã đơn hàng"></el-table-column>
          <el-table-column label="Tổng tiền">
            <template slot-scope="scope">
              <span>{{ scope.row.amount | vnd }}</span>
            </template>
          </el-table-column>
          <el-table-column label="Tạm ứng" width="180">
            <template slot-scope="scope">
              <span>{{ scope.row.deposit_cost | vnd }}</span>
            </template>
          </el-table-column>
          <el-table-column label="Còn lại" width="150">
            <template slot-scope="scope">
              <span>{{ scope.row.debt_cost | vnd }}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    >
      <div class="title-wrap">
        ...
        >
        <img src="@/static/icon/pack/list/show-detail.svg" />
        <span class="show-detail-782fkme" @click="viewOrder(packSelected)"
          >Xem đơn hàng</span
        >
      </div>
      <div class="table-v2-827292abc">
        ...
        >
          ...
          <div v-if="packSelected.is_order == true">
            <div
            ...
            >
              ...
              <div class="icon-item">
                <div class="price-item d-flex flex-column">
                  <span>{{
                    (i.unit_price_cny * hic.exchange_rate) | vnd
                  }}</span>
          ...
          <!--  -->
          <div v-if="packSelected.is_order == false">
            <!-- <div class="table-v2-827292abc-item d-flex">
              <div class="icon-item first">
                <img :src="hic.products.image" alt="" />
                <div class="d-flex flex-column pl-3">
                  <span>{{ hic.products.name }}</span>
                  <span class="sub-12x">{{ hic.products.note }}</span>
                </div>
              </div>
              <div class="icon-item">
                <span>{{ hic.products.quantity }}</span>
              </div>
              <div class="icon-item">
                <div class="price-item d-flex flex-column">
                  <span>{{
                    (hic.products.unit_price_cny * hic.exchange_rate) | vnd
                  }}</span>
                  <span>¥{{ hic.products.unit_price_cny }}</span>
                </div>
              </div>
              <div class="icon-item">
                <div class="price-item d-flex flex-column">
                  <span class="price-red">{{
                    (hic.products.unit_price_cny *
                      hic.products.quantity *
                      hic.exchange_rate)
                      | vnd
                  }}</span>
                  <span class="price-red"
                    >¥{{
                      hic.products.unit_price_cny * hic.products.quantity
                    }}</span
                  >
                </div>
              </div>
            </div> -->
            <div v-for="item in hic.products" :key="item.hic">
              <div class="table-v2-827292abc-item d-flex">
                <div class="icon-item first">
                  <img :src="item.image" alt="" />
                  <div class="d-flex flex-column pl-3">
                    <span>{{ item.name }}</span>
                    <span class="sub-12x">{{ item.note }}</span>
                  </div>
                </div>
                <div class="icon-item">
                ...
                <div class="icon-item">
                  <div class="price-item d-flex flex-column">
                    <span>{{
                      (item.unit_price_cny * hic.exchange_rate) | vnd
                    }}</span>
                    <span>¥{{ item.unit_price_cny }}</span>
                  </div>
                </div>
                <div class="icon-item">
                  <span>{{ item.note }}</span>
                ...
                <div class="icon-item">
                  <div class="price-item d-flex flex-column">
                    <span class="price-red">{{
                      (item.unit_price_cny * item.quantity * hic.exchange_rate)
                        | vnd
                    }}</span>
                    <span class="price-red"
                      >¥{{ item.unit_price_cny * item.quantity }}</span
                    >
                  </div>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    //
    viewOrder(item) {
      if (item.is_order == false) {
        this.$router.push({
          name: "consignment-detail",
          params: { consignment_id: item.order_id },
        });
      } else {
        this.$router.push({
          name: "order-detail",
      ...
      axios.get("package", { params: filter }).then((response) => {
        ...
        switch (response.code) {
          case 200:
            ...
            this.reportsPackToVN = response.data.reports;
            break;
          default:
            this.$notify.error({
              title: "Error",
    ...
    //
    fetchConsig() {
      axios.get("consignment").then((response) => {
        switch (response.code) {
          case 200:
            this.listCon = response.data.items;
            break;
          default:
            this.$notify.error({
              title: "Error",
