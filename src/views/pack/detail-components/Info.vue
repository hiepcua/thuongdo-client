  ...
  <div>
    ...
    <div class="table-element-custom-2022 pack-detail-table-89">
      <el-table :data="tableData" style="width: 100%">
        ...
        <!-- <el-table-column label="Danh mục hàng hóa">
          <span>{{ packDetail.category }}</span>
        </el-table-column> -->
        <el-table-column label="Phí vận chuyển nội địa TQ (vnd)">
          <div class="d-flex flex-column">
            <span>{{ packDetail.china_shipping_cost | vnd }}</span>
          </div>
        </el-table-column>
        <el-table-column label="Phí vận chuyển quốc tế (vnd)">
          <div class="d-flex flex-column">
            <span>{{ packDetail.international_shipping_cost | vnd }}</span>
          </div>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // },
    viewDetail() {
      if (this.packDetail.is_order == false) {
        this.$router.push({
          name: "consignment-detail",
          params: { consignment_id: this.packDetail.order_id },
        });
      } else {
        this.$router.push({
          name: "order-detail",
